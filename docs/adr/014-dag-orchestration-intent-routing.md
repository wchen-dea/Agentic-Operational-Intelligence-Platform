...
1. **`IntentRouter`** (`ai_layer/orchestration/router.py`) — classifies query intent using two-phase matching:
...
3. **`DAGExecutor`** (`ai_layer/orchestration/executor.py`) — executes tiers in topological order:
...
### Neutral / constraints
- DAG nodes are defined in `ai_layer/orchestration/dag.py` with `AgentNode(name, dependencies, agent)`.
