...
The `ai_system/llm.py` module implements five interaction modes:
...
**Model routing** (`ai_system/model_router.py`) selects the optimal model by task complexity:
...
### Positive
- Vendor lock-in is minimal — `ai_system/llm.py` abstracts all calls; swapping to another provider requires changing only that file and `config/settings.py`.
