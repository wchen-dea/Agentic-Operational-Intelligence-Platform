...
The `ai_layer/llm.py` module implements five interaction modes:
...
**Model routing** (`ai_layer/model_router.py`) selects the optimal model by task complexity:
...
### Positive
- Vendor lock-in is minimal — `ai_layer/llm.py` abstracts all calls; swapping to another provider requires changing only that file and `config/settings.py`.
