# ADR-016: Hybrid Context Assembly (Streaming + Vector + Memory)
...
Implement a **`HybridContextAssembler`** (`ai_system/context.py`) that merges three sources into a single `ContextWindow` object consumed by all agents:
...
2. **`LocalHybridSearch`** (`ai_system/rag/retrieval/hybrid_search.py`) — combines:
