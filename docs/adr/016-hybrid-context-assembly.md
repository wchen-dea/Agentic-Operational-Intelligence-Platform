...
Implement a **`HybridContextAssembler`** (`ai_layer/context.py`) that merges three sources into a single `ContextWindow` object consumed by all agents:
...
2. **`LocalHybridSearch`** (`ai_layer/rag/retrieval/hybrid_search.py`) — combines:
